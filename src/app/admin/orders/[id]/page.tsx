...
const PRODUCT_REVIEW_URL = (orderId: string) => `https://printinghouseujjain.in/orders/${encodeURIComponent(orderId)}`;
const GOOGLE_REVIEW_URL = "https://www.google.com/maps/search/?api=1&query=Printing+House+Ujjain%2C+Ujjain%2C+Madhya+Pradesh";
...
function formatTime(value?: string) {
    ...
    return Number.isNaN(date.getTime()) ? value : date.toLocaleTimeString("en-IN", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
    });
...
function buildWhatsAppMessage(
    ...
    switch (status) {
        ...
        case "refunded": {
            ...
            const refundType = options?.refundType === "partial" ? "a partial refund" : "a full refund";
            return [header, `*PAYMENT REFUNDED* 💰`, divider,
                `Hi *${order.customer.name}*,\n\nWe've processed *${refundType}* for your order.`, divider,
                ...
                divider, deliveryDetails + `\nOrder Status: ${normalizeStatus(order.rawStatus)}`, divider,
        ...
        default:
            return [header, `*ORDER UPDATE* 📦`, divider,
                `Hi *${order.customer.name}*, your order #${order.id} has been updated.`, divider,
                orderDetails, divider, paymentDetails, divider,
                `🔎 *Track Your Order:*\n${trackingLink}`, divider, contacts,
            ].join("\n\n");
    }
}
        ...
        try {
            const response = await fetch("/api/update_order", {
                ...
                body: JSON.stringify({
                    order_id: order.id,
                    status,
                    current_status: order.rawStatus,
                }),
            });
            ...
            const messageText = buildWhatsAppMessage(order, status, { cancellationReason });
                ...
                <div className="mx-auto max-w-7xl">
                    ...
                    {(notificationLink || notificationWebLink) && (
                        <div className="mt-3 flex flex-wrap gap-2">
                            {notificationLink && (
                                <a href={notificationLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-[#25D366] px-4 py-2.5 text-sm font-semibold text-white">
                                    <MessageCircle size={16} />WhatsApp App
                                </a>
                            )}
                            {notificationWebLink && (
                                <a href={notificationWebLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-[#25D366] bg-white px-4 py-2.5 text-sm font-semibold text-[#168C46]">
                                    <Monitor size={16} />WhatsApp Web
                                </a>
                            )}
                        </div>
                    )}
