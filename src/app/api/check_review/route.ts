...
export async function POST(request: NextRequest) {
	try {
		...
		const response = await fetch(`${API_URL}/api/check_review`, {
			method: "POST",
			headers: {
		...
		try {
			...
			data = { message: text || "Invalid review status response." };
		}

		return NextResponse.json(data, { status: response.status });
	} catch (error) {
		console.error("Check review proxy error:", error);
		return NextResponse.json({ message: "Unable to check review status." }, { status: 500 });
	}
}
